...
function agregarNumeros(arrayOfNums) {
   ...
   // Tu código:
   let result = arrayOfNums.reduce((acc, elem) => (acc + elem))
   return result
}
...
function numeroMasGrande(arrayOfNums) {
   ...
   // Tu código
   // Con reduce
   let mayor = arrayOfNums.reduce((acc, elem) => {return Math.max(acc, elem)}, arrayOfNums[0])
   return mayor
}
...
function empiezaConNueve(num) {
   ...
   if (!(numStr.includes('.')) && numStr.startsWith('9')) return true
   return false
}
...
function mesesDelAño(array) {
   ...
   // Tu código:
   res = array.map((elem) => {
      if (["Enero", "Marzo", "Noviembre"].includes(elem)) {
         return elem
      }
